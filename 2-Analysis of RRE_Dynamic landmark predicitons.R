...
###############################################################################
### This file contains R code for the analyses of time to relapse 
### and it is inherited from Dynamic Prediction in Clinical Survival Analysis 
...
################################################################
### Dynamic Landmark Predictions using the Progbnostic index ###
################################################################
...
setwd("C:/Users/Owner/Documents/Survival_Analysis/DynPred_JointModels/DynPreds_JM/PhD_Study_1/Article_1/Analysis_of_Relapse/LandmarkModels")


## Regression coefficients (standard errors) in the two-stage Cox regression approach 

##Read in data save in stage 1
xzmat<-read.delim("xzmat.txt")
tt <- sort(unique(xzmat$Obstime[xzmat$status==1]))
xzmat2 <- survSplit(data=xzmat, cut=tt, end="tstop", start="Tstart", event="status")
xzmat2$lnt <- log(xzmat2$time+1)  #time
...
xbeta<-xzmat$xbeta;
zbeta<-xzmat$zbeta;  
xzbeta<-xzmat$xzbeta #this is the super learner. It is a combination of xbeta and zbeta. See below

##Time-fixed cox model on CEPI
...
##Time-varying cox model on CEPI
...
##Time-fixed cox model on the combined index CEPI + GPI
...
##Compute the superlearner

xzmat2$xzbeta <-coef(xzfixed)[1]*xzmat2$zbeta+coef(xzfixed)[2]*xzmat2$xbeta  #the super learner xzbeta is the combination of xbeta and zbeta following Cox regression on the xbeta + zbeta
xzmat$xzbeta <-coef(xzfixed)[1]*xzmat$zbeta+coef(xzfixed)[2]*xzmat$xbeta

##Time-fixed cox model on the super learner
...
##Time-varying cox model on the super learner
...
iplstrat
###clinical only
iplstratclin1 <- coxph(Surv(LM,tstop,status) ~ clin1 +  strata(LM) +cluster(auslongid),robust = TRUE, data=LMdata, method="breslow")
iplstratclin1

iplstratclin2 <- coxph(Surv(LM,tstop,status) ~ clin1 + clin2 + clin3+ strata(LM) + 
                         cluster(auslongid),robust = TRUE, data=LMdata, method="breslow")
iplstratclin2
...
###############################################################################
### Dynamic fixed width predictions  without and with landmark interactions
###############################################################################
...
par(mfrow=c(1,2))
plot(tpred,Fwclin11,type="s",lwd=3,ylim=c(0,0.35),
     xlab="Prediction time (years)",ylab="Probability")
lines(tpred,Fwclin12,type="s",lwd=3,lty=2)
lines(tpred,Fwclin21+0.01,type="s",lwd=3,col="#770060")
lines(tpred,Fwclin22+0.01,type="s",lwd=3,col="#770060",lty=2)
legend("bottomleft",
...
plot(tpred,Fwgen11,type="s",lwd=3,ylim=c(0,0.4),
     xlab="Prediction time (years)",ylab="Probability")
lines(tpred,Fwgen12,type="s",lwd=3,lty=2)
lines(tpred,Fwgen21+0.01,type="s",lwd=3,col="#770060")
lines(tpred,Fwgen22+0.01,type="s",lwd=3,col="#770060",lty=2)
legend("bottomleft",
       c("High risk, landmark fixed","High risk, landmark dependent",
         "Low risk, landmark fixed","Low risk, landmark dependent"),
       lwd=3,lty=c(2,2,1,1),
...
plot(tpred,Fwcomb11,type="s",lwd=3,ylim=c(0,0.45),
     xlab="Prediction time (years)",ylab="Probability")
lines(tpred,Fwcomb12,type="s",lwd=3,lty=2)
lines(tpred,Fwcomb13,type="s",lwd=3,lty=3)
lines(tpred,Fwcomb14,type="s",lwd=3,lty=4)
lines(tpred,Fwcomb21+0.01,type="s",lwd=3,col="#770060")
lines(tpred,Fwcomb22+0.01,type="s",lwd=3,col="#770060",lty=2)
lines(tpred,Fwcomb23+0.01,type="s",lwd=3,col="#770060",lty=3)
lines(tpred,Fwcomb24+0.01,type="s",lwd=3,col="#770060",lty=4)
legend("topleft",
...
legend("topright",c("Landmark fixed","Landmark dependent"),
...
par(mfrow=c(1,2))
plot(pes$time,pes$null,type="s",lwd=3,lty=1,col=8,ylim=c(0.05,0.35),xlab="Time (years)",ylab="Prediction error")
...
lines(pes$time,pes$clin2-0.01,type="s",lwd=3,lty=1,col="#996060")
lines(pes$time,pes$gen2-0.01,type="s",lwd=3,lty=2,col="#996060")
lines(pes$time,pes$comb2-0.01,type="s",lwd=3,lty=3,col="#996060")
legend("bottomleft",c("Null model","clinical model","genetic model","clinico-genotypic model"),
...
text(6,0.20,"Kullback-Leibler",adj=1)
...
plot(pesr$time,abs(pesr$clin1),type="s",lwd=3,lty=1,col=1,ylim=c(0,0.35),xlab="Time (years)",ylab="Prediction error reduction")
...
legend("topleft",c("clinical model","genetic model","clinico-genotypic model"),
       lwd=3,lty=c(1,2,3),col=c(1,"#646060",1),bty="n")
legend("bottomleft",c("Landmark fixed","Landmark dependent"),
       lwd=3,col=c(1,"#996060"),bty="n", cex=0.8)
text(4,0.4,"Kullback-Leibler",adj=1)
...
## With and Without landmark interactions plotted together
par(mfrow=c(1,2))
plot(pes$time,pes$null+mean(pes$clin1)+0.01,type="s",lwd=3,lty=1,col=8,ylim=c(0.05,0.35),xlab="Time (years)",ylab="Prediction error")
lines(pes$time,pes$clin1+0.01,type="s",lwd=3,lty=1,col=1)
lines(pes$time,pes$gen1+0.01,type="s",lwd=3,lty=2,col=1)
lines(pes$time,pes$comb1+0.01,type="s",lwd=3,lty=3,col=1)
lines(pes$time,pes$clin2,type="s",lwd=3,lty=1,col="#996060")
lines(pes$time,pes$gen2,type="s",lwd=3,lty=2,col="#996060")
lines(pes$time,pes$comb2,type="s",lwd=3,lty=3,col="#996060")
legend("bottomleft",c("Null model","clinical model","genetic model","clinico-genotypic model"),
       lwd=3,lty=c(1,1,2,3,1,2,3),col=c(8,1,1,1, "#996060"),bty="n")
legend("topleft",c("Landmark fixed","Landmark dependent"),
       lwd=3,col=c(1,"#996060"),bty="n", cex=0.8)
text(5,0.2,"Brier",adj=1)
...
## With and Without landmark interactions plotted together
plot(pesr$time, abs(pesr$clin1),type="s",lwd=3,lty=1,col=1, ylim=c(0.2,0.83),xlab="Time (years)",ylab="Prediction error reduction")
lines(pesr$time,abs(pesr$gen1),type="s",lwd=3,lty=2,col=1)
lines(pesr$time,abs(pesr$comb1),type="s",lwd=3,lty=3,col=1)
lines(pesr$time,abs(pesr$clin2)+0.01,type="s",lwd=3,lty=1,col="#996060")
lines(pesr$time,abs(pesr$gen2)+0.01,type="s",lwd=3,lty=2,col="#996060")
...
xmat <- xzmat[,c(1,5,6, 7,48:248, 250,253)]
...
for (i in 1:length(LMs)) {
  ...
  nr<-dim(LMdata)[1]
  print(LMdata[1:6,1:8])
  optlam1.LM <- optL1(Surv(LM,tstop,status), LMdata[,6:ni], data=LMdata, fold = 20)
  keep<-names(coefficients(optlam1.LM$fullfit))
...
save(czresults, cxresults, cxzresults,LMzresults, LMxresults, LMxzresults, file="LandmarkSpecific.RData")
